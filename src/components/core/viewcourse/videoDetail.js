...
const VideoDetails = () => {
  ...
  const isFirstVideo = () => {
    // if(videoData[0] === courseSectionData[0]._id)return true;
  ...
  const isLastVideo = () => {
    ...
    if(currentSectionIndx === courseSectionData.length - 1 && currentSubSectionIndx === noOfSubSection){
      return true;
    }
    else return false;
  ...
  return (
    <div className="flex flex-col object-cover lg:w-[1000px]  gap-5 py-5 text-white">
      {!videoData ? (
        <img
          // src={previewSource}
          alt="Preview"
